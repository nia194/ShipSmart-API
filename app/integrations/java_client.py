...
Wraps the shared `httpx.AsyncClient` created in the application lifespan
(`app.bootstrap`) so we do not open a second connection pool. All methods degrade
gracefully: they log + return None on failure rather than raising, because
Python-side advisors should still produce *some* answer if Java is unreachable.
"""
